...
class BarChartPainter extends AxisChartPainter<BarChartData> {
  ...
  void paint(CanvasWrapper canvasWrapper, PaintHolder<BarChartData> holder,
      {double offsetX = 0.0, BarMaxXCallBack? barMaxXCallBack}) {
    ...
    final groupsX = _calculateGroupsX(canvasWrapper.size, data.barGroups,
        data.alignment, holder, canvasWrapper.size,
        offsetX: offsetX, maxXCallBack: barMaxXCallBack);
    ...
    _drawTitles(canvasWrapper, _groupBarsPosition!, holder, drawBottom: false);

    final leftTextsSpace = getLeftOffsetDrawSize(holder);

    canvasWrapper.save();

    canvasWrapper.clipRect(Rect.fromLTWH(leftTextsSpace / 2, 0,
        canvasWrapper.size.width, canvasWrapper.size.height * 1.5));
    _drawBars(canvasWrapper, _groupBarsPosition!, holder);
    _drawTitles(canvasWrapper, _groupBarsPosition!, holder,
        drawBottom: true, drawLeft: false, drawTop: false, drawRight: false);
    ...
    switch (alignment) {
      case BarChartAlignment.start:
        ...
        barGroups.asMap().forEach((i, group) {
          ...
          tempX += group.width + data.groupsSpace;
          totalBarX += group.width + data.groupsSpace;

          if (maxXCallBack != null && i == barGroups.length - 1) {
            if (totalBarX < window.width) {
              maxXCallBack(-1);
            } else {
              maxXCallBack(totalBarX - ((window.width / 4) * 3));
            }
          }
        });
        break;
    ...
    if (drawTop) {
      ...
      if (topTitles.showTitles) {
        for (var index = 0; index < groupBarsPosition.length; index++) {
          ...
          final span =
              TextSpan(style: topTitles.getTextStyles(xValue), text: text);
          final tp = TextPainter(
              text: span,
              textAlign: TextAlign.center,
      ...
      if (rightTitles.showTitles) {
        ...
        while (verticalSeek <= data.maxY) {
          if (rightTitles.checkToShowTitle(
            ...
            final span = TextSpan(
                style: rightTitles.getTextStyles(verticalSeek), text: text);
            final tp = TextPainter(
                text: span,
                textAlign: TextAlign.center,
    ...
    if (drawBottom) {
      ...
      if (bottomTitles.showTitles) {
        ...
        for (var index = 0;
          ...
          final span =
              TextSpan(style: bottomTitles.getTextStyles(xValue), text: text);
          final tp = TextPainter(
              text: span,
              textAlign: TextAlign.center,
  ...
  BarTouchedSpot? handleTouch(
    ...
    if (_groupBarsPosition == null) {
      final groupsX = _calculateGroupsX(
          viewSize, data.barGroups, data.alignment, holder, viewSize);
      _groupBarsPosition =
